module box
    ...
    contains
            ...
            subroutine grid_extra (pos, mass, dens, hsml, extra, ninten, ngx, ngy, ngz, sx, sy, sz, center, box_id, boxed_extra)
                implicit none
                ...
                if (SIZE(extra, 2) .lt. ninten) then
                    WRITE(*,*) "Bad 'extra' input shape:"
                    WRITE(*,*) " ", SIZE(extra, 2), " was given, and not lower than", ninten, " was expected"
                    WRITE(*,*) "Exiting."
                    call EXIT(1)
                end if
...
!                 touch_grid = in_grid
                ...
                PRINT*, " Cantidad de partículas dentro del grid:", COUNT(in_grid)
                PRINT*, "  Dentro/Total = ", COUNT(in_grid) / (1. * SIZE(mass))
                PRINT*, " Cantidad de partículas interactuantes con el grid:", n_useful
                PRINT*, "  Interactuantes/Total = ", n_useful / (1. * SIZE(mass))
...
!                 vol_u = 4. * PI * hsml_u * hsml_u * hsml_u / 3. ! volumen
                ...
                do j = 1, ninten
                    Aj(j,:) = PACK(extra(:,j), touch_grid) * vol_u ! cantidad "extra" (sin normalizar, para intrínsecos)
                end do
                do j = ninten + 1, n_extra
                    Aj(j,:) = PACK(extra(:,j), touch_grid) ! cantidad "extra" (sin normalizar, para extrínsecos)
                end do
...
!                 Sj = Sj * vol_k !! Está comentado porque se hace maualmente a Ak al final
                ...
                WRITE(*,*) "Arreglando boxes vecinos..."
                ...
                WRITE(*,*) "Corrigiendo pesos Ak..."
                do j = 1, ninten
                    Ak(j,:) = Ak(j,:) / vol_k
                end do
                
                ! Dealocatamos
                deallocate(hsml_u, box_ijk_u)
                deallocate(pos_norm_u, mass_u, vol_u)
                deallocate(Sj, Aj)
                ...
                PRINT*, "Guardando normalizaciones Ik..."
